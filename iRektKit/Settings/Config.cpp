...
//			SetupValue(Vars.Legitbot.Weapon[id].Silent, false, GetWeaponName(id), strenc("Silent"));
	...
	SetupValue(Vars.Visuals.SkyChanger, 0, strenc("Visuals"), strenc("Skychanger"));
...
void CConfig::Save()
{
	...
	{
		char szCmd[256];
		sprintf(szCmd, "\\Picoloware\\%s.ini", Vars.Misc.configname);

		folder = std::string( path ) + strenc( "\\Picoloware\\" );
